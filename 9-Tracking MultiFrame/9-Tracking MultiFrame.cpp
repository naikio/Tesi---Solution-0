...
void ConfrontModelAndCurrentBlobs(vector<BlobDistance>& squaredDistances, vector<FloorObject>& modelBlobs, vector<FloorObject>&currentBlobs);
...
void UpdateModel(vector<FloorObject>& model, vector<FloorObject>& currentBlobs, double maxDistance);
	...
	while (!GetAsyncKeyState(VK_ESCAPE)){
		...
		*pointCloud2 = *pointCloud;
		...
		if (modelBlobs.empty()){
			...
			UpdateModel(modelBlobs, currentBlobs, 630);
		}
		...
		for (vector<FloorObject>::iterator i = modelBlobs.begin(); i < modelBlobs.end(); i++){
			...
			//increment not_render_flag
			i->notVisible++;
		}

		bool doneWithRearranging = false;
		while (!doneWithRearranging){
			doneWithRearranging = true;
			for (vector<FloorObject>::iterator i = modelBlobs.begin(); i < modelBlobs.end(); i++){

				if (i->notVisible > 12){
					modelBlobs.erase(i);
					doneWithRearranging = false;
					break;
				}
			}
		}
...
void UpdateModel(vector<FloorObject>& model, vector<FloorObject>& currentBlobs, double maxDistance){
	...
	ConfrontModelAndCurrentBlobs(distances, model, currentBlobs);
	...
	for (vector<BlobDistance>::iterator i = distances.begin(); i < distances.end(); i++){
		...
		if (both_not_found){
			...
			//					matching->positions.push_back(i->box.center);
			matching->notVisible = 0;
