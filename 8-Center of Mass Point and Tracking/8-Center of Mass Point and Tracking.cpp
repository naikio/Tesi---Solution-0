...
typedef struct FloorObject{
	...
	bool operator==(const FloorObject& obj) const{
		return (box.center == obj.box.center && box.size == obj.box.size);
	}
...
typedef struct BlobDistance{
	...
	//Operator <  ---> Sort function
	bool operator<(const BlobDistance& bDist) const{
		return distance < bDist.distance;
	}
...
void PointCloudXYPlaneToMat(PointCloud<PointXYZRGB>::Ptr pointCloud, Mat& floorProjection, float topRightX, float topRightY, float bottomRightX, float bottomRightY, int depthHeight, int depthWidth);
void FindFloorBlobsContours(Mat& floor, int binaryThreshold, vector<FloorObject>& floorBlobs);
void ConfrontCurrentAndPreviousBlobs(vector<BlobDistance>& squaredDistances, vector<FloorObject>& previousBlobs, vector<FloorObject>& currentBlobs);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	vector<FloorObject> previousBlobs;
	vector<FloorObject> finalBlobs;
	...
	while (!GetAsyncKeyState(VK_ESCAPE)){
		...
		/////////////////////////////
		vector<BlobDistance> squaredDistances;
		if (previousBlobs.empty()){
			FindFloorBlobsContours(floor, 20, finalBlobs);
		}
		else {
			//confront old blobs with new one
			FindFloorBlobsContours(floor, 20, currentBlobs);
			if (currentBlobs.empty()){
				vector<FloorObject>().swap(currentBlobs);
				vector<FloorObject>().swap(finalBlobs);
				vector<FloorObject>().swap(previousBlobs);
			}
			ConfrontCurrentAndPreviousBlobs(squaredDistances, previousBlobs, currentBlobs);
			//check distances and identify blobs
			IdentifyFloorBlobs(squaredDistances, previousBlobs, currentBlobs, finalBlobs);
		}
		...
		for (vector<FloorObject>::iterator it = finalBlobs.begin(); it < finalBlobs.end(); it++){
...
#endif
		//Rearrange vectors for next iteration
		finalBlobs.swap(previousBlobs);
		vector<FloorObject>().swap(currentBlobs);
		vector<FloorObject>().swap(finalBlobs);
...
void ConfrontCurrentAndPreviousBlobs(vector<BlobDistance>& squaredDistances, vector<FloorObject>& previousBlobs, vector<FloorObject>&currentBlobs){
	for (int cur_i = 0; cur_i < currentBlobs.size(); cur_i++){
		for (int prev_i = 0; prev_i < previousBlobs.size(); prev_i++){
			double distance = SquaredDistance(currentBlobs[cur_i].box.center, previousBlobs[prev_i].box.center);
			BlobDistance temp;
			temp.distance = distance;
			temp.current = currentBlobs[cur_i];
			temp.previous = previousBlobs[prev_i];
